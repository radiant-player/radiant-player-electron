...
/**
 * Notes with <YouTube> events for this.state or setting doesn't work results to an error stating setting state isn't a function
...
class MiniplayerYoutube extends Component {
    ...
    toggle = (e) => {
        ...
        let isPaused = document.querySelector(".youtube iframe").contentDocument.querySelector('.html5-video-player').classList.contains("paused-mode");
        let isPlayed = document.querySelector(".youtube iframe").contentDocument.querySelector('.html5-video-player').classList.contains("playing-mode");
    ...
    onError() {
        let restriction = "Looks like Radiant Player can't play this video as to grounds of restriction by YouTube. Click the `X` on the top right to continue";

        let check_error = (document.querySelector(".youtube iframe").contentDocument.querySelector('.ytp-error-content-wrap span') === null) ? !1 : !!1;

        if(check_error){
            let error = document.querySelector(".youtube iframe").contentDocument.querySelector('.ytp-error-content-wrap span').textContent.match("restricted");
    ...
    onEnd(){
        return document.querySelector(".youtube a").click();
    }
    ...
    _onReady(e) {
        let player =  document.querySelector(".youtube iframe").contentDocument;//document.querySelector(e.target.a).contentDocument;
        let player_container = player.querySelector(".html5-video-container");
    ...
    render() {
        ...
        const { className, triggerClassName, trigger, youTubeID } = this.props;
        ...
        const renderTriger = (active = this.state.isYouTubeShown) => {
            if(active) {
                return <svg className={"youtube_button " + "active" + " " + miniplayer.shadow} viewBox="0 0 24 24" preserveAspectRatio="xMidYMid meet">
                    <g>
                        <path d="M19 6.41L17.59 5 12 10.59 6.41 5 5 6.41 10.59 12 5 17.59 6.41 19 12 13.41 17.59 19 19 17.59 13.41 12z" />
                    </g>
                </svg>
            } else {
                return <svg className={"youtube_button" + " " + miniplayer.shadow} viewBox="0 0 24 24" preserveAspectRatio="xMidYMid meet">
                    <g>
        ...
        const renderTrigerFullScreen = (active = this.state.isYouTubeFullScreen) => {
            if(active) {
                return <svg className={"youtube_button fullscreen " + "active" + " " + miniplayer.shadow} viewBox="0 0 24 24" preserveAspectRatio="xMidYMid meet">
                    <g>
                ...
                return <svg className={"youtube_button fullscreen" + " " + miniplayer.shadow} viewBox="0 0 24 24" preserveAspectRatio="xMidYMid meet">
                    <g>
        ...
        return (<div onLoad={this.hide} className={youtubeClasses}>
                ...
                <div>
                    <div className={'youtube__content ' + miniplayer.minilayout  + ' ' + miniplayer.flex_center + ' ' + miniplayer.self_center}>
                        <YouTube videoId={youTubeID} opts={this.state.options} onReady={this._onReady} onEnd={this.onEnd} onError={this.onError} onPause={this.onPauseVideo} onPlay={this.onPlayVideo} />
                    </div>
                </div>
            </div>
        );
